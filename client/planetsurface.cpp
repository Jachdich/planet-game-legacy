...
void PlanetSurface::draw(olc::PixelGameEngine * e, CamParams trx, Planet * p) {
    ...
    for (int i = 0; i < p->radius * 2; i++) {
        for (int j = 0; j < p->radius * 2; j++) {
            ...
            drawTile(i, j, rand() % 4 == 0 ? 1 : 0, e, trx, p);
        }
    }
}
...
PlanetSurface::PlanetSurface(Json::Value root) {
	tiles.reserve(root["rad"].asInt());
	for (int i = 0; i < root["rad"].asInt(); i++) {
		tiles.push_back(root["tiles"][i].asInt()); //CURRENTLY SERVER DOESNT SEND OVER RADIUS, PLEASE HELP
	}
}
